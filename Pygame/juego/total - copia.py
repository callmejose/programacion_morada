...
musica_fondo = ["01 - Opening.ogg","02 - BWV 1007 - Prelude.ogg","03 - HWV 56 - Why do the nations so furiously rage together.ogg"]

fin_de_cancion = pygame.USEREVENT + 1
pygame.mixer.music.set_endevent(fin_de_cancion)
pygame.mixer.music.load("./musica/"+musica_fondo[0])
pygame.mixer.music.play()

def aleatorizar_sonido(el_vector,cancion_actual):
    siguiente = random.choice(el_vector)
    while cancion_actual == siguiente:
        siguiente = random.choice(el_vector)
    cancion_actual = siguiente
    pygame.mixer.music.load("./musica/"+siguiente)
    pygame.mixer.music.play()        
...
while not hecho:
    ...
    for evento in pygame.event.get(): # El usuario hizo algo
        ...
        if evento.type == fin_de_cancion:
            aleatorizar_sonido(musica_fondo,musica_fondo[0])
